...
const USDC_MAINNET: Pubkey = Pubkey::new_from_array([198,250,122,243,190,219,173,58,61,101,243,106,171,201,116,49,177,187,228,194,210,246,224,228,124,166,2,3,69,47,93,97]);
const USDC_DEVNET: Pubkey = Pubkey::new_from_array([59,68,44,179,145,33,87,241,58,147,61,1,52,40,45,3,43,95,254,205,1,162,219,241,183,121,6,8,223,0,46,167]);
...
fn check_collateral(m:&Pubkey)->ProgramResult{if *m==USDC_MAINNET || *m==USDC_DEVNET{Ok(())}else{Err(ProgramError::InvalidArgument)}}
...
pub fn process_instruction(program_id:&Pubkey,accounts:&[AccountInfo],data:&[u8])->ProgramResult{
 ...
 match ix {
  StocklanaIx::Initialize{market_seed,resolve_at,fee_bps}=>{
   ...
   check_prestock(underlying.key)?;check_collateral(collateral.key)?;
