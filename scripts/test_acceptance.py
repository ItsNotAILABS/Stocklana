...
assets=json.loads((ROOT/'data'/'prestocks-snapshot.json').read_text(encoding='utf-8'));templates=cat.templates(assets);assert len(templates)==360
...
bc=json.loads((ROOT/'data'/'bounty-coverage.json').read_text(encoding='utf-8'));expected={'derivatives','defi','tools','games','ai_agents','prediction_markets','social_communities_memes','lending_collateral','structured_products','launchpads','automations','simulations','growth_experiments','new'}
...
fit=json.loads((ROOT/'data'/'track-fit.json').read_text(encoding='utf-8'));assert 'programmable tokenized-equity ownership' in fit['wedge'].lower();assertions+=1
...
for f in ['src/accounting_tokens.py','src/token2022-accounting.js','docs/V2-FINANCIAL-SUBSTRATE.md','ACCOUNTING-TOKEN-RESULT.json']:
 assert (ROOT/f).exists();assertions+=1
ar=json.loads((ROOT/'ACCOUNTING-TOKEN-RESULT.json').read_text(encoding='utf-8'));assert ar['status']=='PASS' and ar['assertions']>=400 and ar['coverageRatio']>=1.0;assertions+=3

# Requirement contract has no unclassified/missing status.
req=json.loads((ROOT/'data'/'requirements-ledger.json').read_text(encoding='utf-8'));allowed={'EXECUTED_PROVEN','CODED_EXTERNAL_AUTH_REQUIRED','SOURCE_READY_DEPLOYMENT_REQUIRED'}
...
w=json.loads((ROOT/'WEEK-100-PROOF.json').read_text(encoding='utf-8'));assert w['status']=='PASS';assert w['sessions']==700;assert w['agents']==100;assert not w['failures'];assert w['allMarketsSolvent'];assert w['houseDirectionalExposure']==0.0;assertions+=6
# UI navigation and critical action surfaces exist.
h=(ROOT/'index.html').read_text(encoding='utf-8');nav=set(re.findall(r'data-nav="([^"]+)"',h));views=set(re.findall(r'id="view-([^"]+)"',h));assert nav<=views;assertions+=1
...
rust=(ROOT/'programs/stocklana-market/src/lib.rs').read_text(encoding='utf-8')
