...
def accept(challenge_id,opponent,trade):
    d=_load();c=d['challenges'].get(challenge_id)
    if not c: raise ValueError('challenge_not_found')
    if c['creator']==opponent: raise ValueError('creator_cannot_accept_own_challenge')
    if c['expiresAt']<int(time.time()): c['status']='EXPIRED';_save(d);raise ValueError('challenge_expired')
    if c['status']!='OPEN_FOR_OPPONENT': raise ValueError('challenge_not_open')
    c['opponent']=opponent;c['opponentTrade']=trade;c['matchedAt']=int(time.time());c['status']='MATCHED';_save(d);return c
