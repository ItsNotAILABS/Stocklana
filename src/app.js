...
async async function loadFundingPlan(){
 ...
 try{
   ...
   box.innerHTML=(d.routes||[]).slice(0,7).map((x,i)=>`<button class="funding-route ${selectedFundingRoute===x.id?'selected':''}" data-funding-route="${x.id}" data-funding-symbol="${x.symbol||''}"><span class="route-rank">${i+1}</span><span><strong>${x.label}</strong><small>${x.id==='SPLIT_USDC'?money(x.walletUSDC)+' wallet + '+money(x.vaultUSDC)+' Stocklana':x.id==='PRESTOCK_TO_USDC'?'≈ '+Number(x.estimatedTokensToSell||0).toLocaleString(undefined,{maximumFractionDigits:6})+' '+x.symbol+' estimated':x.ready?'Ready now':'Requires wallet conversion'}</small></span><em>${x.ready?'READY':'CHOOSE'}</em></button>`).join('')||'<small>No funding route can cover this amount yet.</small>';
 ...
 }catch(e){box.innerHTML='<small>Funding plan unavailable.</small>'}
}
