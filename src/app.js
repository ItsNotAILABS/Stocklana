...
function renderMarketFeed(){
 const count=$('#marketCount');if(count)count.textContent=markets.length;
 const feed=$('#marketFeed');if(!feed)return;
 const open=markets.filter(m=>m.status==='OPEN');
 if(!open.length){
   feed.innerHTML=(assets||[]).slice(0,3).map((a,i)=>`<article class="feed-card feed-preview"><div class="top"><div><span class="kicker">PROGRAMMABLE PAYOFF</span><h3>Build a ${a.symbol} payoff market from the live PreStock registry.</h3></div><span class="status-pill">PREVIEW</span></div><div class="feed-meta"><span>Fully collateralized</span><span>0 house exposure</span><span>15 settlement rules</span></div><div class="feed-prices"><button class="quicktrade yes" data-preview-market="${a.symbol}"><small>CREATE</small><b>YES</b></button><button class="quicktrade no" data-preview-market="${a.symbol}"><small>CREATE</small><b>NO</b></button></div></article>`).join('');
   feed.querySelectorAll('[data-preview-market]').forEach(b=>b.onclick=()=>{selected=b.dataset.previewMarket;navigate('lab');renderAssets();buildFields()});
   return
 }
 feed.innerHTML=open.slice(0,9).map((m,i)=>{const y=Math.round((m.yesProbability??.5)*100);return `<article class="feed-card premium-feed-card" data-mid="${m.id}" style="--feed-accent:${i%2?'#8d64ff':'#43e9ff'}"><div class="top"><div><span class="kicker">${(m.rule?.type||'MARKET').replaceAll('_',' ').toUpperCase()}</span><h3>${m.question}</h3></div><span class="status-pill">${m.symbol}</span></div><div class="feed-chart" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><em></em></div><div class="feed-meta"><span>${money(m.volume||0)} real volume</span><span>${m.trades?.length||0} plays</span><span>${m.resolveAt||'live rule'}</span></div><div class="feed-prices"><button class="quicktrade yes" data-side="YES"><small>YES</small><b>${y}%</b></button><button class="quicktrade no" data-side="NO"><small>NO</small><b>${100-y}%</b></button></div></article>`}).join('');
 feed.querySelectorAll('.quicktrade[data-side]').forEach(b=>b.onclick=async()=>{const card=b.closest('[data-mid]');const v=await actionSheet({kicker:'PLAY',title:`Buy ${b.dataset.side}`,copy:'Choose your stake. Stocklana shows the quote before moving money.',confirmLabel:'Preview trade',fields:[{name:'shares',label:'Stake / shares',type:'number',value:'10',min:1,step:1}]});const shares=Number(v?.shares||0);if(shares>0)await trade(card.dataset.mid,b.dataset.side,shares)});
}
...
function renderGameDeck(){
 ...
 if(!rows.length){
   ...
   deck.innerHTML=previews.map((a,i)=>`<article class="game-card game-card-premium preview-game" data-preview-game="${a.symbol}"><div class="game-card-head"><div><span class="status preview">PREVIEW</span><span class="game-symbol">${a.symbol}</span></div><span class="status neutral">COLLATERALIZED</span></div><div class="game-visual"><span class="gv-orb">${a.image?`<img src="${a.image}" alt="">`:a.symbol.slice(0,2)}</span><i class="gv-ring r1"></i><i class="gv-ring r2"></i><b>?</b></div><h3>Will ${a.symbol} finish above its current Stocklana mark this round?</h3><div class="game-pool"><span>Preview stake <b>${money(gameStake)}</b></span><span>max loss visible</span></div><div class="game-sides"><button class="game-side yes" data-preview-side="YES"><span>YES</span><b>3.2×</b><small>example payoff</small></button><button class="game-side no" data-preview-side="NO"><span>NO</span><b>1.4×</b><small>example payoff</small></button></div><div class="game-rule"><span>Fully collateralized</span><span>House exposure 0</span></div></article>`).join('');
   deck.querySelectorAll('[data-preview-side]').forEach(b=>b.onclick=()=>toast('Live game pools load from the Stocklana market API'));
   return
 }
 deck.innerHTML=rows.map((g,i)=>{const gp=g.game||{},y=gp.yes||{},n=gp.no||{},pool=Number(g.collateral||0),yes=Math.round(Number(g.yesProbability??.5)*100),no=100-yes;const a=assets.find(x=>x.symbol===g.symbol);return `<article class="game-card game-card-premium" data-family="${g.family}" data-game-id="${g.id}" style="--game-accent:${i%2?'#8d64ff':'#43e9ff'}"><div class="game-card-head"><div><span class="status">${gameTitle(g.family)}</span><span class="game-symbol">${g.symbol||''}</span></div><span class="status ${g.status==='OPEN'?'live':'neutral'}">${g.status}</span></div><div class="game-visual"><span class="gv-orb">${a?.image?`<img src="${a.image}" alt="">`:String(g.symbol||'M').slice(0,2)}</span><i class="gv-ring r1"></i><i class="gv-ring r2"></i><div class="gv-prob"><small>YES</small><b>${yes}%</b></div></div><h3>${g.question}</h3><div class="game-pool"><span>Real pool <b>${money(pool)}</b></span><span>${g.trades?.length||0} plays</span></div><div class="game-sides"><button class="game-side yes" data-game-side="YES"><span>YES · ${yes}%</span><b>${money(y.payoutIfCorrect||0)}</b><small>payout on ${money(gameStake)} · max loss ${money(y.maxLoss||gameStake)}</small></button><button class="game-side no" data-game-side="NO"><span>NO · ${no}%</span><b>${money(n.payoutIfCorrect||0)}</b><small>payout on ${money(gameStake)} · max loss ${money(n.maxLoss||gameStake)}</small></button></div><button class="game-challenge" data-challenge-game="${g.id}">Challenge someone head-to-head →</button><div class="game-rule"><span>Fully collateralized</span><span>House exposure 0</span><span>Receipt-backed</span></div></article>`}).join('');
...
async function loadGames(){
 ...
 try{const d=await fetch(`/api/games?stake=${encodeURIComponent(gameStake)}&limit=24`,{cache:'no-store'}).then(r=>r.json());games=d.games||[];renderGameDeck();await loadMyChallenges()}catch(e){games=[];renderGameDeck()}
}
